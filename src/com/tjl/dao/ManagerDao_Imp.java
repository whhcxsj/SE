...
public class ManagerDao_Imp implements ManagerDao {
	...
	private static final String SQL_Manager_LOGIN = "select type from manager where uname=? and upass=?";
...
//    查询菜单中根据学院进行查询的实现
	public User select(String academy) {
		...
		try {
			...
			rs = pst.executeQuery();
			System.out.println("学院  年级  上报日期  姓名  联系电话  是否今天从外地返校  是否有疑似症状  是否已经确诊");
			while (rs.next()) {
